...
def render_progress_image(bot_data: Dict, puzzle_key: str, collected_piece_ids: List[str]) -> bytes:
    ...
    try:
        if base_image_path and config.PUZZLES_ROOT.joinpath(base_image_path).exists():
            puzzle_img = Image.open(config.PUZZLES_ROOT.joinpath(base_image_path)).convert("RGBA").resize((img_width, img_height), Image.Resampling.LANCZOS)
        else:
