...
class ModerationCog(commands.Cog, name="Moderation"):
    ...
    async def lock(self, ctx: commands.Context):
        ...
        if not verified_role:
            await ctx.send("❌ Tell Alice that the verified role for unlock/lock is broken", ephemeral=True)
            return
    ...
    async def addrole(self, ctx: commands.Context, role: discord.Role, member: discord.Member):
        ...
        if role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
            await ctx.send("❌ You cannot manage roles that are higher than or equal to your own.", ephemeral=True)
            return
        if role >= ctx.guild.me.top_role:
            await ctx.send("❌ I cannot manage that role because it is higher than or equal to my own.", ephemeral=True)
            return
        ...
        try:
            ...
            await ctx.send("❌ Tell Alice to fix my permissions.", ephemeral=True)
